	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('fangwuxinxi','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('fangwuxinxi','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('fangwuxinxi','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('fangwuxinxi','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('fangwuxinxi','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('fangwuxinxi','报修')" type="success" @click="zaixianbaoxiuCrossAddOrUpdateHandler(scope.row,'cross','','','')">报修</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<zaixianbaoxiuFormModel ref="zaixianbaoxiuFormModelRef" @formModelChange="formModelChange"></zaixianbaoxiuFormModel>
	</div>
</template>
<script setup>
	...
	import zaixianbaoxiuFormModel from '@/views/zaixianbaoxiu/formModel'
	const zaixianbaoxiuFormModelRef = ref(null)
    const zaixianbaoxiuCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			zaixianbaoxiuFormModelRef.value.init(row.id,'cross','报修',row,'fangwuxinxi',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
