			...
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.danyuan" placeholder="单元"
								clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.yuangongzhanghao" 
								placeholder="员工账号"
								>
								<el-option v-for="item in yuangongzhanghaoLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
					<div class="search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('zaixianbaoxiu','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('zaixianbaoxiu','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('zaixianbaoxiu','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('zaixianbaoxiu','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column prop="sfsh" label="审核状态" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="审核" v-if="btnAuth('zaixianbaoxiu','审核')" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('zaixianbaoxiu','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.danyuan&&searchQuery.value.danyuan!=''){
			params['danyuan'] = '%' + searchQuery.value.danyuan + '%'
		}
		if(searchQuery.value.yuangongzhanghao&&searchQuery.value.yuangongzhanghao!=''){
			params['yuangongzhanghao'] = searchQuery.value.yuangongzhanghao
		}
	...
	//搜索
	const yuangongzhanghaoLists = ref([])
	const getyuangongzhanghaoLists = () => {
		context?.$http({
			url: 'option/yuangong/yuangongzhanghao',
			method:'get'
		}).then(res=>{
			yuangongzhanghaoLists.value = res.data.data
		})
	}
	getyuangongzhanghaoLists()
	const searchClick = () => {
	...
	const approvalClick = (row) => {
		let params = {
			...
			baoxiushijian: row.baoxiushijian,
			zhanghao: row.zhanghao,
			xingming: row.xingming,
			weixiuzhuangtai: row.weixiuzhuangtai,
			yuangongzhanghao: row.yuangongzhanghao,
